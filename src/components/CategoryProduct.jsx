...
import { Category, Data } from '../assets/Asserts'
import { Link, useParams } from 'react-router-dom'

const CategoryProduct = () => {
  const{category} = useParams()

  const relatedProduct = Data.filter((p)=> p.cat_name === category)
  console.log(relatedProduct);

  return (
    <>
    <div className='mx-30 font-bold text-2xl mt-10'>{category}</div>
                ...
                return(
                    <div className='mt-10 mx-10'>
                       <Link to={`/more/${item.id}/${item.description}`}>
                        <div className='rounded-sm border-gray-400 border py-3 w-[230px]'>
                            <img src={item.image} alt=""  />
                            <div className='text-gray-400 ps-5'>{item.description}</div>
                            <div className='font-bold text-xl ps-5'>{item.name}</div>
